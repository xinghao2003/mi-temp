...
- `GET /devices/<MAC>?limit=50&since=<timestamp>` – measurement history for the
  given device. `limit` defaults to 50 (maximum 1000 per request) and `since`
  can be used to filter results newer than the supplied Unix timestamp.
- `GET /devices/<MAC>/range?range=24h` – measurement history restricted to the
  ...
  defaults to 1000 results. Supplying `limit=all` (or any number above 1000)
  returns the full result set for the selected range.
